...
class PostListState {
  ...
  /// 详情页暂选、尚未随搜索生效的标签（返回主页时按设置决定是否自动应用）。
  ...
  PostListState copyWith({
    ...
    return PostListState(
      ...
      pendingTags: pendingTags ?? this.pendingTags,
...
class PostListNotifier extends Notifier<PostListState> {
  ...
  String _buildTags() {
    ...
    if (state.pendingTags.isNotEmpty) {
      queryTags.addAll(state.pendingTags.where((s) => s.isNotEmpty));
    }
  ...
  void search(String tags) {
    ...
    state = state.copyWith(
      ...
      // 提交搜索时输入框文本已包含暂选标签，统一并入 currentTags。
      pendingTags: const [],
      totalPages: 0,
      hasReachedEnd: false,
    );
  ...
  /// 详情页点标签：暂选/取消（不立即搜索、不返回主页）。
  void togglePendingTag(String tag) {
    final next = <String>[...state.pendingTags];
    if (next.contains(tag)) {
      next.remove(tag);
    } else {
      next.add(tag);
    }
    state = state.copyWith(pendingTags: next);
  }

  /// 应用暂选标签：并入当前搜索并刷新（返回主页时按设置调用）。
  void applyPendingTags() {
    if (state.pendingTags.isEmpty) return;
    final merged = {
      ...state.currentTags.split(' ').where((s) => s.isNotEmpty),
      ...state.pendingTags,
    }.toList();
    state = state.copyWith(
      currentTags: merged.join(' '),
      pendingTags: const [],
      totalPages: 0,
      hasReachedEnd: false,
    );
  ...
  /// 把当前「实际请求串」（用户标签 + 排序 + 评级）记入搜索历史。
  /// 排序/评级变化时也调用 —— 历史与请求保持同步。
  void _recordSearch() {
